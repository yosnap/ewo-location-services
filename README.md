...
## New Features & Usage (v1.2.0)

### PIN Code in Checkout
- The checkout form now includes a 'PIN Code' field, auto-generated as a 4-digit number. Users can edit this value before submitting. This field is required for opportunity creation.

### API Key Handling
- The API key configured in the plugin settings is now automatically included in all opportunity API requests. If the API key is missing or invalid, opportunity creation will fail (success: false).

### Opportunity Response in localStorage
- After submitting the checkout form, the full API response for the opportunity is stored in localStorage under the key `ewo-opportunity-response`.
- On the Thank You page, this response is logged to the browser console and then removed from localStorage.

### Unified Ad Source Options
- All forms and modals now use the same ad source options for consistency and correct API mapping.

## Shortcodes

- `[ewo_location_services]` — Main location and service search form. Place on the entry page for the user flow.
- `[ewo_installation_form]` — Installation details form. Place on the dedicated installation step page.

**Usage Example:**
```
[ewo_location_services]
[ewo_installation_form]
```

## Modals

- The user details and addons selection modal is triggered from the service/package selection step.
- The callback request modal is triggered from the 'Coverage Available' popup.
- All modals use unified styles and validation logic. Only the checkout form includes the PIN Code field.

## Troubleshooting

- If opportunity creation fails (`success: false`), check that the API key is set and valid in plugin settings.
- Make sure the API endpoints are correctly configured for the selected environment (Development/Production).
- If you do not see the expected response in the Thank You page console, ensure the JS file `ewo-thank-you.js` is loaded and that localStorage is not blocked.
...
- The API Key is **never hardcoded** in any PHP or JavaScript file.
- It is always retrieved from the WordPress admin settings using `get_option('ewo_api_key', '')`.
- The key is injected into frontend JavaScript via the `window.ewoLocationConfig.apiKey` variable, generated dynamically by PHP.
- All API requests (AJAX, backend, and frontend) use this dynamic key, ensuring that changing the key in the admin updates it everywhere.
- No API Key or secret is ever exposed in the codebase or repository, following best security practices.
